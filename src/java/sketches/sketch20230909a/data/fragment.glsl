...
void main()
{
    ...
    float e = 2./min(resolution.x, resolution.y);
    vec3 normal = normalize(vec3(
    domainWarp((st+vec2(e, 0)*NOISE_SCALE)).x-n_val.x,
    e,
    domainWarp((st+vec2(0, e)*NOISE_SCALE)).y-n_val.y));
